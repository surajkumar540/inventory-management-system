...
const StatCard = ({ label, value, icon: Icon, color, delay }) => (
  ...
  >
    <div
      className={`absolute -top-6 -right-6 w-24 h-24 rounded-full opacity-[0.07] ${color.bg}`}
    />
    <div className="flex items-start justify-between">
      <div>
        ...
        <p className={`text-[28px] font-black leading-none ${color.text}`}>
          {value}
        </p>
      </div>
      <div
        className={`w-10 h-10 rounded-xl flex items-center justify-center ${color.iconBg}`}
      >
...
const ProductForm = ({ onSuccess }) => {
  ...
  const inputClass =
    "h-11 px-3.5 text-[13px] text-slate-700 bg-slate-50/80 border border-slate-200 rounded-xl outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-100 focus:bg-white transition-all placeholder:text-slate-300 w-full font-medium";
  const labelClass =
    "text-[10.5px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 block";
              ...
              <form onSubmit={handleSubmit}>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[12px] font-bold text-slate-400">
                        ₹
                      </span>
                      <input
                        name="price"
                  ...
                  <div>
                    ...
                    {preview ? (
                      <div className="h-11 flex items-center gap-2 bg-violet-50 border border-violet-200 rounded-xl px-3">
                        ...
                        />
                        <span className="text-[11px] text-violet-600 font-semibold flex-1 truncate">
                          {fileName}
                        </span>
                        <button
                          type="button"
                          onClick={clearFile}
                          className="w-5 h-5 rounded-full bg-violet-200 flex items-center justify-center text-violet-600 hover:bg-violet-300 transition-colors"
                        >
                          <X size={9} />
                        </button>
                      </div>
                      ...
                      <label className="h-11 flex items-center gap-2 bg-slate-50/80 border border-dashed border-slate-200 rounded-xl px-3.5 cursor-pointer hover:border-violet-400 hover:bg-violet-50/40 transition-all group">
                        ...
                        />
                        <span className="text-[12px] text-slate-400 group-hover:text-violet-500 font-medium transition-colors">
                          Choose image…
                        </span>
                        <input
                          type="file"
                ...
                <div className="mt-5 pt-5 border-t border-slate-100 flex items-center justify-end gap-3">
                  <button
                    ...
                    className="h-10 px-5 text-[12px] font-semibold text-slate-500 bg-slate-100 rounded-xl hover:bg-slate-200 transition-all"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={loading}
                    className="flex items-center gap-2 h-10 px-6 bg-gradient-to-r from-violet-500 to-indigo-600 hover:from-violet-600 hover:to-indigo-700 text-white text-[13px] font-bold rounded-xl transition-all shadow-md shadow-violet-200/60 hover:shadow-violet-300/60 disabled:opacity-60 disabled:cursor-not-allowed"
                  >
                    {loading ? (
    ...
    <div className="bg-white border border-slate-200/80 rounded-2xl shadow-sm overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-6 py-4 border-b border-slate-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400">
              {safeProducts.length} product{safeProducts.length !== 1 ? "s" : ""} total
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 w-full sm:w-60 group focus-within:border-violet-300 focus-within:bg-violet-50/20 transition-all">
          <Search
            size={13}
            className="text-slate-400 group-focus-within:text-violet-400 transition-colors shrink-0"
          />
          <input
            value={search}
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50/60">
                {["Image", "Product", "SKU", "Price", "Stock", ...(isManager ? ["Action"] : [])].map(
                  (h, i) => (
                    <th
                      key={h}
                      className={`text-[10px] text-slate-400 font-bold uppercase tracking-widest py-3 px-4 ${
                        i === 0 ? "pl-6" : ""
                      } ${h === "Action" ? "text-right pr-6" : "text-left"}`}
                    >
                      {h}
                    </th>
                  )
              ...
              <AnimatePresence>
                {filtered.map((p, i) => (
                  ...
                  >
                    ...
                    <td className="py-3 pl-6 pr-4">
                      {p.image ? (
                        <img
                          ...
                          alt={p.name}
                          className="w-11 h-11 object-cover rounded-xl border border-slate-200 shadow-sm group-hover:shadow-md transition-shadow"
                        />
                      ) : (
                    ...
                    {/* Price */}
                    <td className="py-3 px-4">
                      <span className="text-[14px] font-black font-mono text-slate-800">
                    ...
                    {isManager && (
                      <td className="py-3 px-4 pr-6 text-right">
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 h-8 px-3.5 text-[11.5px] font-semibold text-red-500 bg-red-50 border border-red-100 rounded-xl hover:bg-red-100 hover:border-red-200 transition-all disabled:opacity-40 opacity-0 group-hover:opacity-100"
                        >
                          {deletingId === p.id ? (
...
const Products = () => {
  ...
  return (
    <motion.div
      ...
      className="space-y-6 p-6 min-h-screen bg-slate-50/50"
    >
      {/* ── Page Header ── */}
      <div className="flex items-center justify-between">
        ...
        >
          <h1 className="text-[24px] font-black text-slate-800 tracking-tight">
            Products
          </h1>
      ...
      {/* ── Add Product Form (managers only) ── */}
      {isManager && (
        <motion.div
